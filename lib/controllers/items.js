...
module.exports = Router()
  ...
  .get('/:id', authenticate, authorizeItem, async (req, res, next) => {
    try {
      const item = await Item.getById(req.params.id, req.body);
      res.json(item);
    } catch (e) {
      next(e);
    }
  ...
  .get('/', authenticate, async (req, res, next) => {
    try {
      const item = await Item.getAll(req.user.id, req.body);
      res.json(item);
    } catch (e) {
      next(e);
    }
